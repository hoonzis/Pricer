...
type StocksTests() = 
    ...
    member this.``test single lse stock from quandl`` () =
        let ticker,data = MarketData.stock LSE "VOD" (Some startDate) (Some endDate)
    ...
    member this.``test estimation of vol of lse stock - open values are not available, test only Close Vs Close`` () =
        let ticker,data = MarketData.lseData (getEmbeddedResourceFile "lsesinglestock.json")
    ...
    member this.``test estimation of vol of euronext stock - open values are available`` () =
        let ticker,data = MarketData.euroNextData (getEmbeddedResourceFile "euronextsinglestock.json")
        let vol = Stocks.estimateVol CloseVsClose data
    ...
    member this.``test euronext stock info - `` () =
        let stockInfo = Stocks.stockInfo EURONEXT "ATI" (Some startDate) (Some endDate)
